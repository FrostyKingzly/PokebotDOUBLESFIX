...
class ItemUsageManager:
    ...
    def _use_rare_candy(self, player_id: int, pokemon: Dict, item: Dict) -> ItemUseResult:
        ...
        if pokemon.get('level', 1) >= 100:
            return ItemUseResult(False, f"❌ {pokemon['species_name']} is already at max level (100)!")
        ...
        message = f"✨ {pokemon['species_name']} leveled up to **Level {old_level + 1}**!"

        if can_evolve and method == 'level':
            message += f"\n🌟 {pokemon['species_name']} is ready to evolve!"
    ...
    def _use_tm(self, player_id: int, pokemon: Dict, item: Dict, item_id: str) -> ItemUseResult:
        ...
        return ItemUseResult(
            success=True,
            message=f"✨ {pokemon['species_name']} learned **{move_name}**!",
            pokemon_changed=True,
            move_learned=True
        )
    ...
    def _use_evolution_item(self, player_id: int, pokemon: Dict, item: Dict, item_id: str) -> ItemUseResult:
        ...
        if not evolution:
            return ItemUseResult(False, f"❌ {pokemon['species_name']} cannot evolve with this item!")
        ...
        if method == 'stone' and required_stone in item_id.lower():
            ...
            return ItemUseResult(
                success=True,
                message=f"✨ {pokemon['species_name']} is evolving!",
                pokemon_changed=True,
